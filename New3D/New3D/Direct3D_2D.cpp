...
#include "DXErr.h"
Direct3D::Exception::Exception(HRESULT hr, const std::wstring& note, const wchar_t* file, unsigned int line)
	:
	DXException(file, line, note),
	hr(hr)
{}

std::wstring Direct3D::Exception::GetFullMessage() const
{
...
std::wstring Direct3D::Exception::GetErrorName() const
{
	return DXGetErrorString(hr);
}

std::wstring Direct3D::Exception::GetErrorDescription() const
{
...
std::wstring Direct3D::Exception::GetExceptionType() const
{
	return L"Direct3D Graphics Exception";
}
...
// public functions
Direct3D::Direct3D(UINT scrnWidth, UINT scrnHeight, HWND& hwnd, bool vsync, bool fullscreen)
	:
